...
export default function CopiaDeSegurancaBackup() {
    ...
    return (
        <div className="bg-zinc-300 text-black">
            <div className="flex flex-col max-w-[1500px] w-full mx-auto gap-4 p-4 md:p-6 lg:p-10">
                ...
                <TituloMateria texto={`${tecnicoDeInformatica["copia-de-seguranca-backup"].titulo}`} />
                <IntroducaoMateria introducao={tecnicoDeInformatica["copia-de-seguranca-backup"].introducao} />
                <ReproduzirTexto
                    texto={
                        Array.isArray(tecnicoDeInformatica["copia-de-seguranca-backup"].introducao)
                            ? tecnicoDeInformatica["copia-de-seguranca-backup"].introducao.join(' ')
                            : tecnicoDeInformatica["copia-de-seguranca-backup"].introducao
                    }
                />
                <SubmateriasNavegacao arrayDeMaterias={tecnicoDeInformatica["copia-de-seguranca-backup"].submaterias} />

                <ul className="flex flex-col gap-4">
                    {
                        tecnicoDeInformatica["copia-de-seguranca-backup"].submaterias.map((submateria, i) => {
                            return (
                                <li key={i} id={submateria.id} className={`flex flex-col gap-2 p-2 ${tecnicoDeInformatica["copia-de-seguranca-backup"].submaterias.length - 1 == i ? '' : 'border-b-2 border-black'}  pb-8 ${materiasCompletas.includes(submateria.id) ? "bg-green-500" : ""
                                    }`}>
                                    {/* Titulo */}
                ...
                <div className="flex flex-col gap-4 max-w-[800px] ml-auto w-full md:grid md:grid-cols-2">
                    ...
                    <AncoraLink link={`/pages/materias/exercicios/${createSlug(tecnicoDeInformatica["copia-de-seguranca-backup"].id)}`} texto="Realizar Exercicios" />
                </div>
