...
export default function PrincipiosBasicosDaAdministracao() {
    ...
    return (
        <div className="p-2 bg-zinc-300 text-black min-h-[77vh] flex flex-col gap-3">
            ...
            <TituloMateria texto={`${assistenteAdministrativo["principios-basicos-da-administracao-publica"].titulo}`} />
            <IntroducaoMateria introducao={assistenteAdministrativo["principios-basicos-da-administracao-publica"].introducao} />
            <ReproduzirTexto
                texto={
                    Array.isArray(assistenteAdministrativo["principios-basicos-da-administracao-publica"].introducao)
                        ? assistenteAdministrativo["principios-basicos-da-administracao-publica"].introducao.join(' ')
                        : assistenteAdministrativo["principios-basicos-da-administracao-publica"].introducao
                }
            />
            <SubmateriasNavegacao arrayDeMaterias={assistenteAdministrativo["principios-basicos-da-administracao-publica"].submaterias} />

            <ul className="flex flex-col gap-4">
                {
                    assistenteAdministrativo["principios-basicos-da-administracao-publica"].submaterias.map((submateria, i) => {
                        return (
                            <li key={i} id={submateria.id} className={`flex flex-col gap-2 p-2 ${materiasCompletas.includes(submateria.id) ? "bg-green-500" : ""
                                ...
                                {
                                    submateria.imagem != undefined && (
                                        <Imagem enderecoDaImagem={submateria.imagem} />
                                    )
                                }
                                {
