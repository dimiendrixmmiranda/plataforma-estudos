...
export default function CorreioEletronico() {
    ...
    return (
        <div className="bg-zinc-300 text-black">
            <div className="flex flex-col max-w-[1500px] w-full mx-auto gap-4 p-4 md:p-6 lg:p-10">
                ...
                <TituloMateria texto={`${tecnicoDeInformatica["correio-eletronico"].titulo}`} />
                <IntroducaoMateria introducao={tecnicoDeInformatica["correio-eletronico"].introducao} />
                <ReproduzirTexto
                    texto={
                        Array.isArray(tecnicoDeInformatica["correio-eletronico"].introducao)
                            ? tecnicoDeInformatica["correio-eletronico"].introducao.join(' ')
                            : tecnicoDeInformatica["correio-eletronico"].introducao
                    }
                />
                <SubmateriasNavegacao arrayDeMaterias={tecnicoDeInformatica["correio-eletronico"].submaterias} />

                <ul className="flex flex-col gap-4">
                    {
                        tecnicoDeInformatica["correio-eletronico"].submaterias.map((submateria, i) => {
                            return (
                                <li key={i} id={submateria.id} className={`flex flex-col gap-2 p-2 ${tecnicoDeInformatica["correio-eletronico"].submaterias.length - 1 == i ? '' : 'border-b-2 border-black'}  pb-8 ${materiasCompletas.includes(submateria.id) ? "bg-green-500" : ""
                                    }`}>
                                    {/* Titulo */}
                ...
                <div className="flex flex-col gap-4 max-w-[800px] ml-auto w-full md:grid md:grid-cols-2">
                    ...
                    <AncoraLink link={`/pages/materias/exercicios/${createSlug(tecnicoDeInformatica["correio-eletronico"].id)}`} texto="Realizar Exercicios" />
                </div>
