...
export default function AdministracaoDeUsuarios() {
    ...
    return (
        <div className="bg-zinc-300 text-black">
            <div className="flex flex-col max-w-[1500px] w-full mx-auto gap-4 p-4 md:p-6 lg:p-10">
                ...
                <TituloMateria texto={`${tecnicoDeInformatica["administracao-de-usuarios"].titulo}`} />
                <IntroducaoMateria introducao={tecnicoDeInformatica["administracao-de-usuarios"].introducao} />
                <ReproduzirTexto
                    texto={
                        Array.isArray(tecnicoDeInformatica["administracao-de-usuarios"].introducao)
                            ? tecnicoDeInformatica["administracao-de-usuarios"].introducao.join(' ')
                            : tecnicoDeInformatica["administracao-de-usuarios"].introducao
                    }
                />
                <SubmateriasNavegacao arrayDeMaterias={tecnicoDeInformatica["administracao-de-usuarios"].submaterias} />

                <ul className="flex flex-col gap-4">
                    {
                        tecnicoDeInformatica["administracao-de-usuarios"].submaterias.map((submateria, i) => {
                            return (
                                <li key={i} id={submateria.id} className={`flex flex-col gap-2 p-2 ${tecnicoDeInformatica["administracao-de-usuarios"].submaterias.length - 1 == i ? '' : 'border-b-2 border-black'}  pb-8 ${materiasCompletas.includes(submateria.id) ? "bg-green-500" : ""
                                    }`}>
                                    {/* Titulo */}
                ...
                <div className="flex flex-col gap-4 max-w-[800px] ml-auto w-full md:grid md:grid-cols-2">
                    ...
                    <AncoraLink link={`/pages/materias/exercicios/${createSlug(tecnicoDeInformatica["administracao-de-usuarios"].id)}`} texto="Realizar Exercicios" />
                </div>
